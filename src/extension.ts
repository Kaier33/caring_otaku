...
function extractInfo(parm: WeatherData) {
  let array: Array<number> = []
  let weatherNotice = ''
  parm.hourly_forecast.forEach(function (el: Hourly): void {
    if (el.cond.code > 204 && !weatherNotice) {
      weatherNotice = ` , ${
        Number(el.date.substring(8, 2)) - new Date().getDate() > 0 ? '明天' : '今天'
        }${el.date.substring(-5, 2)}点后有${el.cond.txt}`
    }
    array.push(el.tmp)
  })
  const tmpRange = {
